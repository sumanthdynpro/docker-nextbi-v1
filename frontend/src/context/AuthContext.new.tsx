...
import { msalConfig, loginRequest, silentRequest } from '../config/auth.config';
import { loginWithAzure, logout as logoutService, getStoredSession } from '../services/loginService';
import { getToken, getUser, getAuthHeaders, clearAuthData } from '../services/authService';
    ...
    const initializeMsal = async () => {
      try {
        ...
        if (storedSession && storedUser) {
          ...
          setIsAuthenticated(true);
          setAuthMethod(storedSession.authMethod || 'msal');
          console.log(`Restored user session from localStorage using ${storedSession.authMethod || 'msal'} auth`);
          setLoading(false);
          setError(null);
        } else {
